...
(function () {
  ...
  (function wish() {
    ...
    var COLORS = ["#21ce99", "#ffc9a3", "#ffffff", "#7ae3c3", "#ff9ec4"];
    ...
    function tick() {
      ...
      for (var i = bits.length - 1; i >= 0; i--) {
        ...
        b.rot += b.vr;
        b.life -= 0.0085;
        if (b.life <= 0 || b.y > canvas.clientHeight + 40) { bits.splice(i, 1); continue; }
        ctx.save();
    ...
    function makeWish() {
      ...
      if (hint) {
        hint.textContent = DATA.wishMade || "wish granted";
        hint.classList.add("is-done");
      }
      if (!reduced) burst(r.left + r.width / 2, r.top + r.height / 3);
      window.setTimeout(function () { cake.classList.remove("is-out"); }, 2600);
    }

    cake.addEventListener("click", makeWish);
    cake.addEventListener("keydown", function (e) {
      if (e.key === "Enter" || e.key === " ") { e.preventDefault(); makeWish(); }
    });
